...
var
  lihat_inv_stokk: Tlihat_inv_stokk;
  idxx,tgl_trans,keterangan,kode_barang,nama_barang,spesf,satuan,qtyy,infoo:string;

implementation

uses koneksi, input_inv_stok, utama, login;

{$R *.dfm}

{ Tlihat_inv_stokk }

procedure Tlihat_inv_stokk.baref;
begin
  input_inv_stokk.mrefresh;
  kosong;
end;

procedure Tlihat_inv_stokk.koneksi;
begin
  with koneksii do
  begin
    koneksi;
      ...
      begin
        ...
        SQL.Clear;
        SQL.Text:='SELECT * FROM tbl_log_stok';
        Active:=true;
      end;
  end;
end;

procedure Tlihat_inv_stokk.kosong;
begin
...
procedure Tlihat_inv_stokk.mrefr_log;
begin
...
procedure Tlihat_inv_stokk.simp_log;
begin
   koneksii.putus;
with koneksii.qr_log do
  begin
...
procedure Tlihat_inv_stokk.tampil;
{tgl_trans,keterangan,kode_barang,nama_barang,spesf,satuan,qtyy:}
begin
...
procedure Tlihat_inv_stokk.btn_tambahClick(Sender: TObject);
begin
...
procedure Tlihat_inv_stokk.cmb_cariClick(Sender: TObject);
begin
...
procedure Tlihat_inv_stokk.DBGrid1CellClick(Column: TColumn);
begin
...
procedure Tlihat_inv_stokk.edit_cariChange(Sender: TObject);
begin
with koneksii.qr_logstok do
  begin
    ...
    else if cmb_cari.Text = 'Masuk/Keluar' then
    begin
      sql.Clear;
      sql.Add('select * from tbl_log_stok where `KETERANGAN` like "%' +edit_cari.Text + '%"');
      Open;
    end
...
procedure Tlihat_inv_stokk.btn_editClick(Sender: TObject);
begin
kosong;
//  close;
  kebenaran:=false;
...
procedure Tlihat_inv_stokk.btn_hapusClick(Sender: TObject);
begin
...
procedure Tlihat_inv_stokk.btn_batalClick(Sender: TObject);
begin
baref;
end;

procedure Tlihat_inv_stokk.btn_tutupClick(Sender: TObject);
begin
kosong;
  close;
end;

procedure Tlihat_inv_stokk.FormShow(Sender: TObject);
begin
  baref;
end;

procedure Tlihat_inv_stokk.sSpeedButton1Click(Sender: TObject);
begin
  btn_batalClick(btn_batal);
  kebenaran:=true;
  with input_inv_stokk do
    begin
      urutkanc;
      tampil_cm3;
      lbl_tampung.Caption:='Barang Keluar';
      lbl_barmas_barkel.Caption:='Barang Keluar';
      btn_simpbar.Caption:='&Simpan';
      btn_simpbar.Enabled:=true;
      kosong;
